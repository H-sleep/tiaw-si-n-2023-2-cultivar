...
cadastrarBtn.addEventListener('click', function(){
    alert("Créditos ao Pedro pela página de cadastro.");
});

pesquisaPlantasEl.addEventListener('keyup', pesquisa_plantas);
conectarBtn.addEventListener('click', function(){
  alert("Na próxima sprint: quando clickado, o botao levará para uma nova página de cadastro");
});
...
function gera_cards(){
  ...
  for(let i = 0; i < dados.length; i++){
    strHtml += `<div class="plants"><div id="plant${i+1}" class="imagens_plantas"><img src="${dados[i].foto}"></div><div class="text-div"><p>${dados[i].resumo}</p><div class="botoes"><a href="cultivar.html?${i+1}"><button id="cultivar-${i+1}">CULTIVAR</button></a><a href="#"><button id="receita-${i+1}">RECEITAS</button></a></div></div></div>`;
